...
- Fixed bug: `Target class [Apxiaoxv\Modules\Commands\] does not exist.`
...
- New File Activator feature. [PR #790](https://github.com/apxiaoxv/laravel-modules/pull/790) from @ryssbowh
...
- Alternate way to define the namespace of modules in [PR #776](https://github.com/apxiaoxv/laravel-modules/pull/776) by @daison12006013
...
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Apxiaoxv\Modules\Module`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Apxiaoxv\Modules\Module`.
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Apxiaoxv\Modules\FileRepository`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Apxiaoxv\Modules\FileRepository`.
- **BREAKING** Removed the `__get` magic method on the `\Apxiaoxv\Modules\Module` class. Use `get()` or `json()->get()` instead.
...
- Laravel mix configuration (https://apxiaoxv.com/laravel-modules/v3/basic-usage/compiling-assets)
...
- Module generation namespace now works with `StudlyCase` ([Issue #14](https://github.com/apxiaoxv/laravel-modules/issues/14))
